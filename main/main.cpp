...
void lihat_buku_semua_pinjaman() {
    ...
    if (!file) {
        eror("Belom Ada Buku Yang dI Pinjam");
        system("pause");
        menuadmin();
    }
...
void lihat_judul_cari(string& kata_kunci) {
    ...
    file.close();
    if (!found) {
        eror("Belom Ada Buku Yang Tersedia");
    }
}
...
void lihat_genre() {
    ...
    if (!ada_buku) {
        cout << RED<<"Tidak Ada Buku Dengan Genre "<< genre_terpilih<<RESET << endl;
        return;
    }
